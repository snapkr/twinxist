...
class DjangoSpider(DjangoBaseSpider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.scraper.pagination_type == 'C':
            self.rules = ( Rule (SgmlLinkExtractor(allow=(),restrict_xpaths=(self.scraper.pagination_crawelerrule_xpath,)), callback="parse_page", follow= True), )
            super(DjangoSpider, self)._compile_rules()
    ...
    def _set_start_urls(self, scrape_url):
        ...
        if self.scraper.pagination_type not in ('N', 'C'):
        ...
        if self.scraper.pagination_type == 'C':
            if not self.scraper.pagination_crawelerrule_xpath:
                raise CloseSpider('Please provide a xpath rule for the crawler!')
        ...
        if self.scraper.pagination_type in ('N', 'C'):
            self.start_urls.append(scrape_url)
    ...
    def _scrape_item_attr(self, scraper_elem):
        if(self.from_detail_page == scraper_elem.from_detail_page):
            ...
            #print "Testprint", scraper_elem.scraped_obj_attr.name, scraper_elem.x_path
    ...
    def parse_page(self, response):
        ...
        for obj in base_objects:
            ...
            if(item and url_name in item):
                ...
                if cnt > 0:
                    item[url_name] = 'DOUBLE' + item[url_name][0]
